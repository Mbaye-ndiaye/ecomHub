            ...
            {(tbody ?? []).length !== 0 &&
                tbody.map((td, index) => (
                    <tr className="text-gray-700 hover:bg-gray-50 dark:hover:bg-gray-100" key={index}>
                        <td className="px-4 py-3 border">
                            <div className="flex items-center text-sm">
                                <div className="md:relative w-8 h-8 mr-3 rounded-full md:block">
                                    <img className="object-cover w-full h-full rounded-full" src={td.image} alt="" loading="lazy" />
                                    <div className="md:absolute inset-0 rounded-full shadow-inner" aria-hidden="true"></div>
                                </div>
                            </div>
                        </td>
                        {td.nom || td.nom === 0 ?
                            <td className="px-4 py-5 text-xl text-center border">{td.nom}</td> : null
                        }
                        {td.description &&
                            <td className="px-4 py-5 text-xl text-center border">{td.description}</td>
                        }
                        {td.quantite &&
                        ...
                        {td.prix &&
                            <td className="px-4 py-5 text-xl text-center border">{td.prix.toLocaleString('de-DE')}</td>
                        }
                        {actions &&
