...
export default function HeaderTable({ filtre, nomAjout, body, test }) {
  ...
  const breadcrumbs = pathnames.map((name, index) => {
    ...
    const textStyle = isLast
      ...
      : "capitalize text-gray-700 hover:text-blue-600 dark:text-gray-400 dark:hover:text-blue";
    const separator = (
      <span
        key={`separator-${index}`}
        className="mx-1 text-gray-700 hover:text-blue-600 dark:text-gray-400 dark:hover:text-white"
      >
        {">"}
      </span>
    );
      ...
      <React.Fragment key={`breadcrumb-${index}`}>
        ...
        {name === "admin" ? (
          <li className="inline-flex items-center">
            <Link
              to="/admin/dashboard"
              className={`inline-flex items-center text-sm ${textStyle}`}
            >
              {name}
            </Link>
          </li>
      ...
      <nav className="flex mb-3" aria-label="Breadcrumb">
        <ol className="inline-flex flex-wrap items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
          <li>
            <Link
              to="/"
              className="inline-flex gap-1 items-center text-sm font-medium text-gray-700 hover:text-blue-600 dark:text-gray-400 dark:hover:text-blue"
            >
              <AiFillHome />
              Home
            </Link>
