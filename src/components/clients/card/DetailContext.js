...
const DetailsCard = () => {
  ...
  const produit = products.find((item) => {
    return item.id === id;
  });

  // const {title, prix} = produit
  ...
  const reduction = produit ? produit?.title : produit?.title ? produit?.title : 0;
  const prixAAjouter = Math.floor(produit?.price - produit?.price * (reduction / 100));
  const handleAddToCart = () => {
    const produitAAjouter = { ...produit, price: prixAAjouter };
